...
export const productModuleRegistry: ProductModuleDescriptor[] = [
  {
    ...
    capabilities: ['parameter-input', 'geometry-2d', 'assembly-validation', 'product-3d', 'layout', 'pdf'],
    gates: [
  ...
  },
  {
    ...
    capabilities: ['parameter-input', 'geometry-2d', 'product-3d', 'layout', 'pdf'],
    gates: [
  ...
  },
  {
    ...
    readiness: 'prototype-required',
    summary: 'Параметрическая геометрия, 3D-визуализация и PDF доступны; требуется физическая приёмка конструкции.',
    capabilities: ['parameter-input', 'geometry-2d', 'product-3d', 'layout', 'pdf'],
    gates: [
      { id: 'knowledge-reviewed', label: 'Базовая конструкция описана', passed: true },
      { id: 'input-complete', label: 'Параметры документа и материала заполнены', passed: true },
      { id: 'geometry-validated', label: 'Геометрия проходит автоматические проверки', passed: true },
      { id: 'assembly-validated', label: 'Сборка проверена мастером', passed: false },
      { id: 'prototype-built', label: 'Контрольная обложка изготовлена', passed: false },
      { id: 'master-approved', label: 'Мастер утвердил методику', passed: false },
    ],
  },
  {
